...
public class Command implements Callable<Void> {
     ...
     */
    public Command(InstanceManager pIManager, String[] pCommand, ClientManager pClient) {
        ClientManager client = pClient;
        PlayerManager player = client.getPlayerManager();;
        InstanceManager iManager = pIManager;
        CoreDatabase coreDatabase = iManager.getCoreDatabase();
        ServerManager server = iManager.getServerManager();
        Email email = iManager.getEmail();
        ...
        if (pCommand.length > 10) {
            iManager.printError(client.getUsername() + " versucht, den Server zu hacken (Befehlsarray zu lang)");
            client.disconnect(true);
        } else if (!correctStringLenth) {
            iManager.printError(client.getUsername() + " versucht, den Server zu hacken (Mindestens ein Befehl ist zu lang)");
            client.disconnect(true);
        } else {
                ...
                switch (pCommand[0]) {
                    ...
                    case Constants.LOGIN:
                        if (state == 0) {
                            ...
                            if (login == 1) {//Hier die Überprüfung ob die Login-Daten richtig sind (Username und PW)
                                if (!coreDatabase.getOnline(pCommand[1])) {//Hier die Überprüfung, ob er schon angemeldet ist
                                    if (coreDatabase.getMailConfirmed(pCommand[1])) {//Hier die Überprüfung, ob er seine E-Mailadresse schon bestätigt hat
                                        iManager.printLine("Erfolgreicher Login von " + pCommand[1]);
                                        ...
                                        iManager.printLine("Fehlgeschlagener Login-Versuch aufgrund fehlender Email-verifizierung");//Darf eig. nicht sein, weil man erst in die DB eingetragen wird, wenn man die Email confirmed hat
                                    }
                                } else {
                                    iManager.printLine("Session von " + pCommand[1] + " läuft schon.");
                                    String[] command = {Constants.ALREADYLOGGEDIN};
                                    client.sendObject(command);
                                }
                            } else if (login == 0) {
                                iManager.printLine("Fehlgeschlagener Login von " + pCommand[1]);
                                String[] command = {Constants.LOGINUNSUCCESSFUL};
                                client.sendObject(command);
                            } else {
                                iManager.printLine("Versuchter SQL-Injection-Angriff von dem vermeintlichen " + pCommand[1] + " (" + client.getIP() + ")");
                    ...
                    case Constants.REGISTER://{register, username, firstname, lastname, pw1, pw2, email, language}
                        if (state == 0) {
                            ...
                            if (coreDatabase.isAccountAlreadyAssigned(pCommand[1], pCommand[6])) {//Prüfung, ob Account bereits mit Usernamen oder Email schoin reigstriert ist
                                iManager.printLine("Account von " + pCommand[1] + " bereits registriert.");
                                ...
                                for (String error : errorList) {
                                    iManager.printError("Registierungsfehler: " + error);
                                }
                            } else {
                                client.setUsername(pCommand[1]);
                                ...
                                iManager.printLine("Sende Bestätigungsmail mit dem Code " + verificationCode);
                    ...
                    case Constants.EMAILCODE:
                        if (state == 0) {
                            ...
                            if (pCommand[1].equals("empty") || pCommand[1].length() < 8) {//Er hat einen dummen Fehler gemacht
                                iManager.printLine(client.getUsername() + " hat keinen Code eingegeben oder er war zu kurz (" + pCommand[1] + "), aber auf OK geklickt -> neue Codeanfrage schicken");
                                if (!(verificationCode.equals(""))) {
                                    ...
                                    iManager.printLine("Sende erneut Bestätigungsmail mit dem Code " + newValidationCode);
                                }
                            } else if (pCommand[1].equals("cancel")) {//Er möchte abbrechen
                                iManager.printLine(client.getUsername() + " hat auf Abbrechen geklickt -> Registrierung abbrechen");
                                ...
                                iManager.printLine("Der Emailcode stimmt überein -> erfolgreiche Registrierung von " + parameter[1] + " (" + parameter[2] + " " + parameter[3] + ")");
                                ...
                                iManager.printLine("Eingegebener E-Mailcode von " + client.getUsername() + " stimmt nicht überein: " + pCommand[1] + " ungleich " + verificationCode);
                                if (!(verificationCode.equals(""))) {
                                    String[] command = {Constants.EMAILCODENOTVALID};
                                    client.sendObject(command);
                                } else {
                    ...
                    case Constants.REQUESTNEWPASSWORD:
                        ...
                        if (state == 0) {
                            try {
                                ...
                                iManager.printLine("Sende Passwort-Vergessenmail mit dem Code " + verificationCode2 + " an " + pCommand[1]);
                                String[] command = {Constants.SHOWPASSWORDCONFIRMATION};
                                client.sendObject(command);
                            } catch (Exception e) {
                                iManager.printLine("Eingegebene Emailadresse existiert nicht: " + pCommand[1]);
                        ...
                        if (state == 0) {
                            ...
                            if (!pCommand[1].equals(verificationCode3)) {
                                if (!(verificationCode3.equals(""))) {
                                    iManager.printLine("Der eingegebene PasswortVergessenCode stimmt nicht überein (" + pCommand[1] + " != " + verificationCode3 + ")");
                                    ...
                                    iManager.printLine("Sende erneut Bestätigungsmail mit dem Code " + newValidationCode2);
                                }
                            } else if (!pCommand[2].equals(pCommand[3])) {
                                iManager.printLine("Die eingegebenen Passwörter stimmen nicht überein (" + pCommand[2] + " != " + pCommand[3] + ")");
                                String[] command = {Constants.PASSWORDSNOTEQUAL};
                                client.sendObject(command);
                            } else {
                                String email2 = client.getEmail();
                                String username = coreDatabase.getUsername(email2);
                                iManager.printLine("Setze neues Passwort für " + username);
